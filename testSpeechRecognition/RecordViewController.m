...
//
//  Created by lisongtao on 16/8/15.
...
//        [alert show];
        [self speechRonWithUrl:mp3FilePath];
    }
}
...
- (void)speechRonWithUrl:(NSURL *)url {
    self.textView.text = @"语音识别中......请稍候";
    //1.创建本地化标识符
...
//    NSURL *url =[[NSBundle mainBundle] URLForResource:path withExtension:nil];
    NSURL *trueUrl = [NSURL fileURLWithPath:url.absoluteString];
    //4.将资源包中获取的url 传递给 request 对象
    SFSpeechURLRecognitionRequest *res =[[SFSpeechURLRecognitionRequest alloc] initWithURL:trueUrl];
    
    //5.发送一个请求
    [sf recognitionTaskWithRequest:res resultHandler:^(SFSpeechRecognitionResult * _Nullable result, NSError * _Nullable error) {
        if (error!=nil) {
            NSLog(@"!!!语音识别解析失败,%@",error);
            self.textView.text = @"语音识别解析失败";
        }
        else
        {
            //解析正确
